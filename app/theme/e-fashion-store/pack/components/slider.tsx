...
export default function Slider({ products }: { products: ProductCardProps[] }) {
  ...
  const getNextActive = (value: string) => {
    if (activeId !== -1 && activeId < products.length-1 ) products[activeId+1].title
  ...
  return (
    <div className='relative w-1/2 -right-[25%] p-1'>
      {products.map((product) => {
        ...
        return (
          <ProductCard
            className={`absolute top-0 duration-700 ${isActive ? activeStyle : normStyle}`}
            src={product.src}
            title={product.title}
            price={product.price}
          />
      ...
      <div className='absolute bottom-0 left-[50%] -translate-x-[50%] flex justify-center items-center gap-10'>
        <button className='w-16 h-16 rounded-full shadow-sm bg-black flex justify-center items-center' onClick={() => setActive(prev => getNextActive(prev))} disabled={activeId === 1}>
          <BsStar className='text-base text-white' />
        </button>
        <button className='w-16 h-16 rounded-full shadow-sm bg-black flex justify-center items-center' onClick={() => setActive(prev => getPrevActive(prev))} disabled={activeId === products.length - 1}>
          <BsStar className='text-base text-white' />
        </button>
      </div>
    </div>
