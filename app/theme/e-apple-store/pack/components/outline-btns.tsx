export default function OutlineButtons({options, setOptions, col}: {options: {}, setOptions: (prev: {}) => void, col: boolean}) {
  ...
  return (
    <div className={`flex ${col && 'flex-col'} items-center justify-start gap-5`}>
      {Object.keys(options).map((option => (<OutlineButton key={option} setSelected={(value) => setSelectedHandler(value, option)}>{option}</OutlineButton>))}
    </div>
  )
}
